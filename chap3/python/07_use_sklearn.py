...
def main():
    ...
    X = np.matrix(np.column_stack([x ** i for i in range(2, 4)] + [np.sin(x)]))
    ...
    alpha_lasso, coefs_lasso, _ = lasso_path(X, y, eps=1e-3, fit_intercept=False)
    ...
    plt.savefig("sk_simple.png", dpi=300)

    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    # Prepare Plot
    plt.figure(figsize=(10,6), dpi=300)
    plt.title(r"Gaussian Lasso Path", fontsize=16)
    plt.xscale('log')
    plt.xlabel(r'$\log{\lambda}$', fontsize=14)
    plt.ylabel(r'coeff', fontsize=14)

    # Plot with Legends
    for i in range(X.shape[1]):
        plt.plot(alpha_lasso, coefs_lasso[i], label=str(i))

    # Other options
    plt.legend(fontsize=12)

    plt.savefig("sk_lasso_path.png", dpi=300)
