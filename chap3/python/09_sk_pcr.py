...
def main():
    ...
    #X = np.matrix(np.column_stack([x ** i for i in range(2, 7)]))
    ...
    pipe = Pipeline(steps)
    pca = pipe.set_params(pca__n_components=4)
    model = pca.fit(X, y)
    y_pcr = model.predict(X)
    ...
    plt.plot(x, y_pcr, color='black', label=r'PCR(4)')

    # Other options
    plt.legend(fontsize=12)

    plt.savefig("pcr_sk_simple_4.png", dpi=300)
